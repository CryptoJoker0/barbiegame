...
declare global { interface Window { phantom?: { solana?: Provider }; backpack?: Provider; x1?: Provider; x1Wallet?: Provider } }

function providers() { if (typeof window === 'undefined') return []; const list: Provider[] = []; if (window.phantom?.solana) list.push({ ...window.phantom.solana, name: 'Phantom' }); if (window.backpack) list.push({ ...window.backpack, name: 'Backpack' }); if (window.x1 || window.x1Wallet) list.push({ ...(window.x1 || window.x1Wallet), name: 'X1 Web Wallet' }); return list }
...
export function X1WalletPanel({ onPaid }: { onPaid: (signature: string) => void }) {
  ...
  useEffect(() => { const refresh = () => setAvailable(providers()); refresh(); window.addEventListener('load', refresh); window.addEventListener('focus', refresh); const timer = window.setInterval(refresh, 1000); return () => { window.removeEventListener('load', refresh); window.removeEventListener('focus', refresh); window.clearInterval(timer) } }, [])
